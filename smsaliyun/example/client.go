...
func main() {
	client, err := rpc.Dial("unix", socket_file)
	if err != nil {
		log.Fatal(err)
	}
	...
	if reply.Success {
		...
		go func() {
			...
			if err != nil {
				log.Print("no.1 example messager about ALARM failed because error")
				log.Print(err)
			} else if !reply.Success {
				log.Print("no.1 example messager about ALARM failed because reply.Success false")
				log.Print(reply.Msg)
			}
			wg.Done()
		}()
		go func() {
			...
			if err != nil {
				log.Print("no.2 example messager about VERIFY failed because error")
				log.Print(err)
			} else if !reply.Success {
				log.Print("no.2 example messager about VERIFY failed because reply.Success false")
				log.Print(reply.Msg)
			}
			wg.Done()
		}()
